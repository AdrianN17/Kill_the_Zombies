...
function entidades:clear()
	...
	self.collider:remove(self.player.point)
...
function entidades:arduino()
	...
	if datos~= nil then
		...
		if split[2]<0 then
			p.direccion.w=true
		elseif split[2]>0 then
			p.direccion.s=true
		end

		p.radio=split[3]+math.pi/2

		if split[4]==0 then
			p.estado.correr=true
		elseif split[4]==1 then
			p.estado.correr=false
		end

		if split[5]==0 and not self.delay then
			self.delay=true
			p.estado.recarga=false

			p:change_weapon()

			self.timer_player:after(0.5, function()
				self.delay=false
			end)
		end

		if split[6]==0 then
			if p.estado.vida then
				p:recarga()
			else
				self.menu=true
			end
		...
		if split[7]==0 then
			...
			p.estado.disparo=false
		end

		if split[4]==0 and split[5]==0 and split[6]==0  then
			self.pausar=true
		end
